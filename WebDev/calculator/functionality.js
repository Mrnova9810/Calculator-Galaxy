...
let dispalyText = document.querySelector(".p2");
...
buttons.forEach(button =>{
    button.addEventListener("click",()=>{
        if(button.innerText == "="){
            // calcucate value
            // find  calculate *
            // keep track of preview oprator.

            // then divide
            // +
            // -

            expression = calcucate(expression);
            dispalyText.innerText = expression;
            ...
            if(isOperator(s,i)){   // handle all cases
                ...
                if(isMust(s, i)){ // means * , /
                    ...
                    if(Do){  // previews operation is  * , /   --> do operation
                        ...
                        // second Number have ,not possible  --> with + , - opt2  if 2nd operation == +,- then then its will add that number to first 1
